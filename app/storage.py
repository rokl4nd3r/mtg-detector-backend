...
_DATASET_ID_SAFE = re.compile(r"[^a-zA-Z0-9._-]+")
...
class Storage:
    ...
    def make_dataset_id(self, device: str, seq: int | None, capture_id: str | None = None) -> Tuple[str, str, str]:
        ...
        cid = sanitize_dataset_id(capture_id or "")
        if cid:
            return cid, dev, seq6
    ...
    def save_pair(self, front_upload, back_upload, grade: str, dataset_id: str) -> Tuple[SavedFile, SavedFile]:
        ...
        if not dataset_id:
            raise ValueError("dataset_id is empty after sanitization")
